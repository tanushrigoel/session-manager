chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.local.set({ timerEndTime: 0, timerMinutes: 0, youtubeTabId: null });
});
  ...
  if (message.action === 'startTimer') {
    chrome.storage.local.get(['timerMinutes'], (data) => {
      ...
      chrome.storage.local.set({ timerEndTime: endTime }, () => {
        ...
        console.log('Timer started for', data.timerMinutes, 'minutes. Alarm set for', new Date(endTime).toLocaleTimeString());
      });
    });
